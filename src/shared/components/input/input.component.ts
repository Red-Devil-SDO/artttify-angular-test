...
export class InputComponent implements OnInit {
  ...
  handleChange(event: Event) {
    if (this.type === "checkbox") {
      this.formGroup.patchValue({
        [this.id]: (event.target as HTMLInputElement)?.checked || false,
      });
    }
  ...
  ngOnInit() {
    ...
    const classesWrapper = [
      `${this.type === "checkbox" ? "checkbox" : "input"}-wrapper`,
    ];
    ...
    if ((Validators as unknown as Record<string, ValidatorFn>)[this.type]) {
      validators.push(
        (Validators as unknown as Record<string, ValidatorFn>)[this.type],
      );
    }

    this.formGroup.addControl(
      this.id,
      new FormControl(this.type === "checkbox" ? false : "", validators),
    );
  }
}
